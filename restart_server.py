...
from src.core import signal,argparse,time,sys,os,errno,threading,subprocess,settings, log_event, is_posix, is_windows

if is_windows():
    import wmi
    #win32gui is used in the raw python file version
    import win32gui
    import win32process
    from win32api import GetUserNameEx
    from src.core import isUserAdmin, runAsAdmin
...
class SVCMgr():
    ...
    def is_service_active(self)->tuple[bool,str,str]:
        '''
        checks to see if process is running already using wmi.Returns a tuple of bool,pid,msg
        '''
        if is_windows():
